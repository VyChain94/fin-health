...
export default function FinancialFreedom() {
  const { latestReport, loading } = useFinancialReports();
  const [customLevelTargets, setCustomLevelTargets] = useState<Record<LevelKey, number> | null>(null);
  
  const financialData: FinancialData = latestReport || {
    income: {
      earned1: 0,
      earned2: 0,
      realEstate: 0,
  ...
  const currentPassiveIncome = totalPassive + totalPortfolio;
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-secondary/10 to-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <FinancialFreedomTracker 
          currentAssets={currentPassiveIncome}
